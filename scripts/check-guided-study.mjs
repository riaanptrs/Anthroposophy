...
assert.equal(Object.keys(counts).length,168);assert.ok(Object.values(counts).every(c=>c===2));
for(const p of ['docs/index.html','docs/pt/index.html'])assert.equal((fs.readFileSync(p,'utf8').match(/class="course-card"/g)||[]).length,8,p);
...
console.log('Passed: 168 bilingual lesson pairs, eight course cards, guided sequence, note controls, lab coverage and static fallback.');
