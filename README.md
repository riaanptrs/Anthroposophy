...
`node scripts/build-all.mjs` (or `node scripts/build-lessons.mjs`) builds all nine courses in order. `node scripts/check-site.mjs` checks all **308 pages**, their links and anchors, language pairs, required learning sections and content completeness. The source comparison and editorial history remain in `content/higher-worlds-reading-review.md` and `content/higher-worlds-course-plan.md`.
