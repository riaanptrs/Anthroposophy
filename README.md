...
The introduction and selected lessons now include original bilingual connections with *The Philosophy of Freedom* and the teaching method from Brian's two preface lectures. Edit these in `content/philosophy-of-freedom-connections.mjs`; both course builders apply them without changing the main books' reading references. Lesson 0 includes a responsive two-question map and four expandable inquiry steps. GA 9 Lesson 18 now asks students to reconstruct and check a chain of reasoning. The separate GA 4 course now publishes the reviewed material through Chapter 14: orientation and 20 lessons, in both languages.

The site now contains **eight bilingual reading courses**: *Theosophy* (orientation, 21 book lessons and synthesis), *How to Know Higher Worlds* (orientation and 18 lessons including synthesis), *The Philosophy of Freedom* (orientation and 20 lessons through Chapter 14), *According to Luke* (orientation, ten lecture lessons and synthesis), *Colour* (orientation, twelve lecture lessons and final portfolio), *The Four Temperaments* (orientation, nine thematic sections and synthesis), and Gilbert Childs’s *Understand Your Temperament!* (orientation, nine chapter lessons, two appendix lessons and synthesis), followed by Hermann Koepke’s *Encountering the Self* (seventeen lessons covering the main sections and four appendices). Course 2 starts at `docs/higher-worlds/index.html`, with its Portuguese counterpart at `docs/pt/higher-worlds/index.html`. The homepages link all eight courses, and the final GA 9 lesson leads into GA 10.
...
`node scripts/build-all.mjs` (or `node scripts/build-lessons.mjs`) builds all eight courses in order. `node scripts/check-site.mjs` checks all **276 pages**, their links and anchors, language pairs, required learning sections and content completeness. The source comparison and editorial history remain in `content/higher-worlds-reading-review.md` and `content/higher-worlds-course-plan.md`.
